...
答：sync数据包送达服务器网卡后，网卡通过DMA将数据包存放在网卡接受队列（ring buffer），然后发起硬中断通知内核，
内核在硬中断处理函数中，将数据迁移到内核缓存SK_BUFF，然后在bottom-half阶段（异步内核线程）按TCP/IP协议栈规范解析该数据包。
...
同时，CPU不高可能是因为bottom-half阶段的异步内核线程数量和执行时间有限制，不会占满CPU（不然可能导致正常情况下时用户线程饿死的情况）。
